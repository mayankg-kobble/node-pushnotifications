...
const _require = require('./constants'),
      ...
      GCM_MAX_TTL = _require.GCM_MAX_TTL;

const ttlFromExpiry = R.compose(R.min(GCM_MAX_TTL), expiry => expiry - Math.floor(Date.now() / 1000));
...
const sendGCM = (regIds, data, settings) => {
  ...
  const message = new gcm.Message({
    ...
    fcm_options: data.fcm_options || {
      analytics_label: data.title
    }
  });
  let chunk = 0;
